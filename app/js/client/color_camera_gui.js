module.exports.initGUIControllers = function(tracker) {
    // GUI Controllers
    ...
    var parametersFolder = gui.addFolder('Tracking parameters');
    ...
    gridFolder.add(window.grid, 'horizontal', 3, 30, 1);
    gridFolder.add(window.grid, 'vertical', 3, 30, 1);

    var midiFolder = gui.addFolder("Midi");

    midiFolder.add(window.midi, 'retrigger');
    midiFolder.add(window.midi, 'blipMode');
    midiFolder.add(window.midi, 'blipLength',1,500,1);

    console.log(window.midi_outputs_names);
    var midiOutController = midiFolder.add(window.midi, 'output_name', window.midi_outputs_names).setValue(window.midi_outputs_names[0]);
    midiOutController.onFinishChange(function(){
        var newOutputName = midiOutController.getValue();
        window.setMidiOutputByName(newOutputName);
    });
